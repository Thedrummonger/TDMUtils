...
namespace TDMUtils
{
    public static class Utility
    {
        ...
        public static T LoadObjectFromFileOrDefault<T>(string FilePath, T Default, bool WriteDefaultToFileIfError)
        {
            ...
            {
                try { 
                    if (Path.GetExtension(FilePath).ToLower().In(".json", ".txt")) { result = DeserializeJsonFile<T>(FilePath); }
                    else if (Path.GetExtension(FilePath).ToLower().In(".yaml")) { result = DeserializeYAMLFile<T>(FilePath); }
                    else if (Path.GetExtension(FilePath).ToLower().In(".csv")) { result = DeserializeCSVFile<T>(FilePath); }
                    else
                    {
